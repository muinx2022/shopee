...
internal sealed class ProductNameRewriteRunner
{
        ...
        CancellationToken ct)
    {
        ...
        {
            // Lỗi HTTP (429/5xx/4xx) lặp lại y nguyên ở mọi cỡ batch — chia nhỏ chỉ tăng
            // số request (tệ hơn khi đang bị rate limit). Fail rõ ràng sau khi đã retry.
            throw;
        }
        catch (InvalidOperationException ex)
        {
            ...
            {
                log($"⚠️ Parse 1 tên thất bại — dùng heuristic tách tên. ({Shorten(ex.Message)})");
                return [InferProductNameStructure(productNames[0])];
            }

            var middle = productNames.Count / 2;
            log($"✂️ Parse batch {productNames.Count} tên lỗi — chia đôi thử lại ({middle}+{productNames.Count - middle}). ({Shorten(ex.Message)})");
    ...
    /// <summary>
    /// Retry chung cho mọi request OpenAI: bắt cả lỗi mạng (HttpRequestException) và
    /// timeout HttpClient (TaskCanceledException không phải do user dừng) — trước đây
    /// hai loại này lọt qua retry làm chết run ngay lần lỗi đầu tiên.
    /// 429/5xx chờ lâu hơn (15s/30s) để qua rate limit thay vì 2s/4s.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        for (var attempt = 1; attempt <= OpenAiMaxRetries; attempt++)
        {
            ...
            catch (Exception ex) when (IsTransientOpenAiError(ex, ct))
            {
                ...
                log($"⚠️ OpenAI {label} lỗi (lần {attempt}/{OpenAiMaxRetries}): {Shorten(ex.Message)} — thử lại sau {delay.TotalSeconds:0}s.");
        ...
        CancellationToken ct)
    {
        ...
        {
            // Lỗi HTTP lặp lại y nguyên ở mọi cỡ batch — không split (xem chú thích ở parse).
            throw;
        }
        catch (InvalidOperationException ex)
        {
            if (products.Count <= 1)
            {
                ...
                log($"⚠️ Rewrite 1 sản phẩm thất bại — giữ mô tả gốc. ({Shorten(ex.Message)})");
                return [products[0].Description ?? ""];
            }

            var middle = products.Count / 2;
            log($"✂️ Rewrite batch {products.Count} sản phẩm lỗi — chia đôi thử lại ({middle}+{products.Count - middle}). ({Shorten(ex.Message)})");
