...
internal static class BigSellerCrawlHelper
{
    ...
    public static async Task<bool> GoToCrawlPageAsync(IPage page, bool forceReload = false, string? targetUrl = null, Action<string>? log = null)
    {
        ...
        for (var attempt = 1; attempt <= 3; attempt++)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    log?.Invoke($"Crawl List d� m?, b? qua l?i ph?: {ex.Message}");
                    await Task.Delay(2000);
                    return true;
                }
    ...
    public static async Task<bool> SelectClaimedTabByTextAsync(IPage page, Action<string>? log = null)
    {
        try
        {
            ...
            {
                case "clicked":
                    log?.Invoke("Da chon tab Da nhan.");
                    await Task.Delay(1500);
                    await WaitForCrawlListContentAsync(page, 8000);
                    return true;
                case "already-active":
                    log?.Invoke("Tab Da nhan da duoc chon san.");
                    return true;
                case "no-items":
                    log?.Invoke("Khong tim thay container tab tren trang.");
                    return false;
                default:
                    log?.Invoke($"Khong tim thay tab Da nhan ({result}).");
                    return false;
            }
        }
        catch (Exception ex)
        {
            log?.Invoke($"Chon tab Da nhan loi: {ex.Message}");
            return false;
        }
    }
    ...
    public static async Task<bool> ClickNextCrawlPageAsync(IPage page, Action<string>? log = null)
    {
        try
        {
            ...
            if (!clicked)
            {
                log?.Invoke("Khong con trang tiep theo.");
                return false;
            }

            log?.Invoke("Chuyen sang trang tiep theo...");
            await Task.Delay(1500);
        ...
        {
            log?.Invoke($"Chuyen trang tiep theo loi: {ex.Message}");
            return false;
        }
    }
    ...
    public static async Task DismissPostImportDialogsAsync(IPage page, Action<string>? log = null)
    {
        try
        {
            var draftLink = page.Locator("a.has_underline:has-text('H?p nh�p')").First;
            if (await draftLink.IsVisibleAsync(new() { Timeout = 3000 }))
            {
                log?.Invoke("B? qua link H?p nh�p, d�ng h?p tho?i...");
