...
internal static class ExtensionRunnerAutomation
{
            ...
            if (!cdpReachable)
            {
                ...
                if ((DateTime.UtcNow - cdpUnreachableSince.Value).TotalSeconds >= CdpUnreachableTimeoutSeconds)
                    throw new InvalidOperationException(
                        ...
                        "Brave c� th? d� t?t � h�y nh?n Stop r?i Start l?i instance.");
        ...
        bool forceNewPopup = false)
    {
        ...
        try
        {
            ...
            await SendCdpAsync(browser, 50, "Target.createTarget", new { url = popupUrl, background = true }, ct);
    ...
    private static Task<List<string>> DiscoverRunnerExtensionIdsAsync(
        int cdpPort,
        DirectoryInfo profileRoot,
        CancellationToken ct)
    {
    ...
    private static List<string> DiscoverExtensionIdsFromProfile(DirectoryInfo profileRoot)
    {
        ...
        var preferencesPath = Path.Combine(profileRoot.FullName, "Default", "Preferences");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (msg.Contains("No tab with id", StringComparison.OrdinalIgnoreCase))
                return new ScrapeStepResult(false, false, false, false, "Tab d� d�ng � s? m? tab m?i ? bu?c sau.", null, link);
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            val.ValueKind == JsonValueKind.Object)
        {
            ...
            log(
                last is > 0
                    ? $"Extension d� d?ng � xong d�ng {last}" + (cur is > 0 ? $", dang d?ng t?i {cur}" : "") +
                      (string.IsNullOrWhiteSpace(sheet) ? "" : $", sheet \"{sheet}\"")
                    : "Extension d� d?ng.");
            return;
        }

        log("Extension d� nh?n l?nh d?ng.");
    ...
    public static async Task<int> ResolveEndRowAsync(string sheet, int startRow, CancellationToken ct)
    {
        ...
        if (total < startRow)
            throw new InvalidOperationException($"T? d�ng {startRow} vu?t qu� s? d�ng sheet ({total}).");
        return total;
    }
