...
internal sealed class BigSellerImportToStoreRunner : IAsyncDisposable
{
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        await BigSellerCookieImporter.ImportFromFileAsync(
            ...
            navigateUrl: BigSellerCrawlHelper.ResolveCrawlUrl(_settings.CrawlUrl),
        ...
        if (_browser is null)
            throw new InvalidOperationException("Kh�ng k?t n?i du?c Brave qua CDP. Ki?m tra BigSeller profile d� dang nh?p chua.");

        var context = _browser.Contexts.FirstOrDefault()
            ?? throw new InvalidOperationException("Brave chua co browser context.");

        var page = await FindBigSellerPageAsync(context, cancellationToken)
            ?? throw new InvalidOperationException("Khong tim thay tab BigSeller.");
        ...
        if (!await BigSellerCrawlHelper.GoToCrawlPageAsync(page, forceReload: true, targetUrl: crawlUrl, log: _log))
            throw new InvalidOperationException("Khong mo duoc Crawl List.");

        await SelectSourceTabIfNeededAsync(page, cancellationToken);

        _log(new string('=', 50));
        _log("BAT DAU IMPORT TO STORE");
        _log(new string('=', 50));
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            loopCount++;
            _log("");
            _log($"Vong #{loopCount} | SP thu {currentIndex + 1} tren Crawl List");
            ...
            {
                _log($"Trang BigSeller dang reload/ngon ngu thay doi, thu lai: {ex.Message}");
                await DelayAsync(3000, cancellationToken);
                await BigSellerCrawlHelper.GoToCrawlPageAsync(page, targetUrl: crawlUrl, log: _log);
                await SelectSourceTabIfNeededAsync(page, cancellationToken);
                continue;
            }
            _log($"Crawl List co {count} san pham.");
            ...
            if (count == 0)
            {
                ...
                _log($"Het san pham. Doi {_settings.ListingReloadSeconds}s...");
            ...
            if (_settings.ImportFromClaimedTab)
            {
                ...
                importCount++;
                _log($"Da Import to Stores #{importCount}.");
                ...
                if (await RemoveClaimedImportRowAsync(page, currentIndex))
                    _log("Da xoa dong vua import khoi bang hien tai.");
                else
                    _log("Khong xoa duoc dong vua import khoi bang hien tai.");

                _log("Import dong tiep theo trong tab Da nhan...");
            ...
            importCount++;
            _log($"�� Import to Stores (#{importCount}) - kh�ng v�o H?p nh�p.");
    ...
    private async Task SelectSourceTabIfNeededAsync(IPage page, CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= 3; attempt++)
        {
            ...
            _log($"Tab Da nhan chua chon duoc (lan {attempt}/3), cho trang load...");
            await DelayAsync(2000, cancellationToken);

            if (attempt == 2)
            {
                _log("Reload trang de thu lai chon tab...");
        ...
        _log("Canh bao: Khong chon duoc tab Da nhan, tiep tuc vong lap ke tiep...");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= 3; attempt++)
        {
            ...
            try
            {
                if (attempt > 1)
                {
                    _log($"Thu lai import dong #{currentIndex + 1} lan {attempt}/3...");
                    await BigSellerCrawlHelper.DismissPostImportDialogsAsync(page, _log);
                    await BigSellerCrawlHelper.GoToCrawlPageAsync(page, targetUrl: crawlUrl, log: _log);
                    await SelectSourceTabIfNeededAsync(page, cancellationToken);
                    await DelayAsync(1000, cancellationToken);
                ...
                if (string.IsNullOrWhiteSpace(claimedProductName))
                    claimedProductName = $"Dong {currentIndex + 1}";
            ...
            {
                lastError = ex;
                _log($"Import dong #{currentIndex + 1} loi lan {attempt}/3: {ex.Message}");
                if (attempt < 3)
                {
        ...
        throw lastError ?? new InvalidOperationException($"Khong import duoc dong #{currentIndex + 1} sau 3 lan thu.");
    }
    ...
    private async Task SelectImportShopAndConfirmAsync(ILocator modal, string shopName)
    {
        ...
        var selectedLabel = await modal.EvaluateAsync<string>(
            @"(root, targetShop) => {
                ...
                // L\u1ea5y t\u1ea5t c\u1ea3 checkbox wrapper trong modal
                const allWrappers = Array.from(root.querySelectorAll(
                    'label.ant-checkbox-wrapper, .ant-checkbox-wrapper, label:has(input[type=checkbox])'));

                // X\u00e1c \u0111\u1ecbnh holder cho shop m\u1ee5c ti\u00eau
                let targetHolder = null;
                for (const w of allWrappers) {
                    if (normalize(w.textContent).includes(target)) {
                        targetHolder = w;
                        break;
                    }
                }
                if (!targetHolder) {
                    const candidates = Array.from(root.querySelectorAll('label, span, div'))
                        .filter(item => normalize(item.textContent).includes(target));
                    for (const item of candidates) {
                        const h = item.closest('label') || item.closest('.ant-checkbox-wrapper') || item.closest('li') || item.parentElement;
                        if (h && h.querySelector('.ant-checkbox, input[type=checkbox]')) { targetHolder = h; break; }
                    }
                }
                if (!targetHolder && allWrappers.length === 1) targetHolder = allWrappers[0];
                if (!targetHolder) throw new Error(`Khong tim thay shop import: ${targetShop}`);

                const isChecked = w => {
                    const inp = w.querySelector('input[type=checkbox]');
                    const box = w.querySelector('.ant-checkbox');
                    return !!(inp?.checked || box?.classList.contains('ant-checkbox-checked') || w.classList.contains('ant-checkbox-wrapper-checked'));
                };
                const clickCheckbox = w => {
                    const t = w.querySelector('.ant-checkbox-inner') || w.querySelector('.ant-checkbox') || w.querySelector('input[type=checkbox]') || w;
                    t.click();
                };

                // Untick t\u1ea5t c\u1ea3 shop KH\u00d4NG ph\u1ea3i m\u1ee5c ti\u00eau
                for (const w of allWrappers) {
                    if (w === targetHolder) continue;
                    if (isChecked(w)) clickCheckbox(w);
                }

                // Tick shop m\u1ee5c ti\u00eau n\u1ebfu ch\u01b0a \u0111\u01b0\u1ee3c ch\u1ecdn
                if (!isChecked(targetHolder)) clickCheckbox(targetHolder);

                return (targetHolder.textContent || targetShop).replace(/\s+/g, ' ').trim();
            }",
            shopName);
    ...
    private static Task<bool> IsImportShopCheckedAsync(ILocator modal, string shopName) =>
        modal.EvaluateAsync<bool>(
            @"(root, targetShop) => {
                ...
                const target = normalize(targetShop);
                const candidates = Array.from(root.querySelectorAll('label, .ant-checkbox-wrapper, span, div'))
                    .filter(item => normalize(item.textContent).includes(target));
                for (const item of candidates) {
                    const holder = item.closest('label') ||
                        item.closest('.ant-checkbox-wrapper') ||
                        item.closest('li') ||
                        item.parentElement;
                    if (!holder) continue;
                    const input = holder.querySelector('input[type=checkbox]');
                    const box = holder.querySelector('.ant-checkbox');
                    if (input?.checked || box?.classList.contains('ant-checkbox-checked') || holder.classList.contains('ant-checkbox-wrapper-checked'))
                        return true;
                }
                return false;
            }",
