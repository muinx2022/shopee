...
_openWithShopeeAccountCheck = new CheckBox
    ...
    public void SetBusy(bool busy, string? statusHint = null)
    {
        if (InvokeRequired)
        {
            BeginInvoke(() => SetBusy(busy, statusHint));
            return;
        }
        ...
        Func<string> getSourceUserData)
    {
        ...
        try
        {
            if (config is null)
            {
                ...
                _statusLabel.Text = "Ch?n instance b�n tr�i";
    ...
    private void OnExtensionProgressSynced()
    {
        if (InvokeRequired)
        {
            BeginInvoke(ApplyExtensionProgressSyncedUi);
            return;
        }
    ...
    /// <summary>N?p l?i danh s�ch sheet t? workbook (g?i khi m? form / ch?n instance).</summary>
    ...
    /// <summary>Ch?n sheet trong dropdown; n?u chua c� trong danh s�ch th� th�m v�o d? v?n hi?n th?.</summary>
    ...
    /// <summary>N?p danh s�ch sheet t? workbook qua API /sheets v�o dropdown.</summary>
