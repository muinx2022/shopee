...
internal static class LauncherRunnerLoop
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (items.Count == 0)
            throw new InvalidOperationException("Kh�ng c� link h?p l? trong kho?ng d�ng d� ch?n.");
            ...
            for (var index = 0; index < items.Count; index++)
            {
                ...
                if (step.Captcha && !step.ScrapeOk)
                {
                    ...
                    config.LastRunnerMessage =
                        step.Message ?? $"Dung vi captcha - {config.DisplayName}, dong {rowNumber} (SKU: {sku}).";
                    await PushDisplayStateAsync(
                        cdpPort, profileRoot, config, sheet, startRow, endRow.Value,
                        index + 1, totalLinks, step.TabId, cancellationToken).ConfigureAwait(false);
                    onProgress();
                ...
                {
                    if (step.Captcha)
                        log($"Da giai captcha va click scrape - dong {rowNumber}.");
                        ...
                        for (var attempt = 0; attempt <= MaxDownloadRetries; attempt++)
                        {
                            try
                            {
                                ...
                                var savedMsg = string.IsNullOrWhiteSpace(videoPath)
                                    ? $"�� t?i {sku}.mp4 � d�ng {rowNumber}"
                                    : $"�� t?i {sku}.mp4\n? {videoPath}";
                                config.LastRunnerMessage = savedMsg;
                                log($"�� t?i {sku}.mp4 � d�ng {rowNumber}.");
                ...
                config.LastRunnerMessage = videoOk
                    ? $"Xong d�ng {rowNumber} � d� t?i video."
                    : $"Xong dòng {rowNumber}.";
                ...
                if (index < items.Count - 1)
                {
                    ...
                    if (tabId is not null)
                    {
                        ...
                        if (check.Waited)
                        {
                            log($"�� gi?i captcha tru?c link ti?p theo � d�ng {rowNumber}.");
                            await TryShowOverlayAsync(
                                cdpPort, profileRoot, tabId,
                                $"�� gi?i captcha � chu?n b? m? link ti?p theo (d�ng {nextRow})�",
                                cancellationToken).ConfigureAwait(false);
                        }
                    }
        ...
        catch (OperationCanceledException)
        {
            ...
            config.LastRunnerMessage = "�� d?ng t? Multi Brave Manager.";
            log(config.LastRunnerMessage);
