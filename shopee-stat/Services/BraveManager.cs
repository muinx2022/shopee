        ...
        if (string.IsNullOrWhiteSpace(bravePath) || !File.Exists(bravePath))
            bravePath = DetectBravePath()
                ?? throw new FileNotFoundException(
                    "Không tìm thấy brave.exe. Cài đặt đường dẫn tại Tab Cấu hình API.");

        var extPath = Path.GetFullPath(Path.Combine(
            Path.GetDirectoryName(Application.ExecutablePath) ?? ".", "shopee-stat-ext"));

        if (!Directory.Exists(extPath))
            throw new DirectoryNotFoundException($"Không tìm thấy extension: {extPath}");
        ...
        try
        {
            foreach (var p in Process.GetProcessesByName("brave"))
            {
                try
                {
                    ...
                    if (commandLine.Contains(fullProfileDir, StringComparison.OrdinalIgnoreCase))
